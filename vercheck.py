...
def latestkKernel(v):
	...
	product_list.get(v)
	print('searching for latest kernel version:\nhttps://scc.suse.com/api/package_search/packages?product_id={0}'.format(v))
	refined_data = search_package(v, package_name, False)
	try:
		#print('latest version for ' + package_name + ' is ' + refined_data[0]['version'] + '-' + refined_data[0]['release'])
		lkv = str((refined_data[0]['version'] + '-' + refined_data[0]['release']))
	except IndexError:
		print('could not find any version for package ' + package_name)
